...
const WalletIco = (a: boolean) => <svg viewBox="0 0 24 24" fill={a?'currentColor':'none'} stroke="currentColor" strokeWidth={2}><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M16 12h2"/><circle cx="17" cy="12" r="1" fill="currentColor"/></svg>
const StarIco = (a: boolean) => <svg viewBox="0 0 24 24" fill={a?'currentColor':'none'} stroke="currentColor" strokeWidth={2}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>

export function CreatorTabs() {
  return <Tabs tabs={[
    { path:'/', label:'Главная', icon: HomeIco },
    { path:'/offers', label:'Офферы', icon: OfferIco },
    { path:'/bonuses', label:'Бонусы', icon: StarIco },
...
export function BusinessTabs() {
  return <Tabs tabs={[
    ...
    { path:'/projects', label:'Проекты', icon: ProjIco },
    { path:'/balance', label:'Баланс', icon: WalletIco },
    { path:'/profile', label:'Профиль', icon: ProfIco },
  ]} />
}
